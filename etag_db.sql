...
create table alunos (id bigint primary key auto_increment, nome varchar(255) not null, cpf varchar(11) not null unique, bairro varchar(100), cidade varchar(100), telefone varchar(9), status enum('Ativos', 'Inativos', 'Todos') default 'Ativos', criado_em timestamp not null default now(), editado_em timestamp not null default now() ON UPDATE now())engine=InnoDB default charset=utf8;
